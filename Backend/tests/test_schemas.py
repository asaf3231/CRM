...
EXPECTED_TOOL_NAMES = [
    ...
    "secured_calculator",
    "build_icp_document",   # Stage 10 — L1 ICP Builder (tool count 8 → 9)
    "discover_contacts",    # Stage 12 — L5b Profile Expander (tool count 9 → 10)
]
...
class TestS0:
    def test_exactly_8_schemas(self):
        """S0: len(TOOL_SCHEMAS) == 10 (was 9; Stage 12 added discover_contacts)."""
        assert len(main.TOOL_SCHEMAS) == 10, (
            f"Expected 10 schemas, got {len(main.TOOL_SCHEMAS)}"
        )
