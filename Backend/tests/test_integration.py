...
class TestINT1SubdomainRouting:
    ...
    def test_int1b_only_tool7_references_subdomain_in_main(self):
        ...
        # Functions allowed to reference OUTREACH_SUBDOMAIN:
        # - request_reactfirst_pdf: the core outbound PDF tool (INT1 original).
        # - dispatch_outreach: the L6a outreach dispatcher (INT1 extension — OUT2,
        #   Stage 13).  Both must ONLY contact OUTREACH_SUBDOMAIN; no other host.
        # - _check_pdf_health: GW4 gateway helper (may reference patterns).
        disallowed = referencing_functions - {
            "request_reactfirst_pdf",
            "dispatch_outreach",   # Stage 13 L6a INT1 extension (OUT2)
            "_check_pdf_health",   # GW4 helper — may reference patterns
        }
        assert not disallowed, (
            f"INT1: Only request_reactfirst_pdf and dispatch_outreach may reference "
            f"OUTREACH_SUBDOMAIN. Disallowed functions found: {sorted(disallowed)}"
        )
...
class TestH1PinnedDependencies:
    ...
    # crm_store added in Stage 11 (Phase 2 mini-CRM workspace)
    # db added in Stage D2 (Phase 4 shared Mongo connection layer — its own pymongo/mongomock imports are pinned)
    LOCAL_MODULES = {"main", "lead_store", "rag_engine", "crm_store", "db"}
